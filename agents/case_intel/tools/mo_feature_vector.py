...
def _load_real_case_mo_vectors() -> Dict[str, Dict[str, Any]]:
    ...
    if os.path.exists(json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            ...
            logger.info(f"Loaded {len(vectors)} real case MO Feature Vectors from database.")
    return vectors

CASE_MO_VECTORS: Dict[str, Dict[str, Any]] = _load_real_case_mo_vectors()
...
async def execute_mo_feature_vector(fir_id: str = "FIR-2026-001") -> Dict[str, Any]:
    ...
    target_case = CASE_MO_VECTORS.get(fir_id)
    if not target_case:
        # Check partial FIR ID match (e.g. FIR-2026-001 vs FIR-2026-0001)
        for cid, case_data in CASE_MO_VECTORS.items():
            if fir_id in cid or cid in fir_id:
                target_case = case_data
                break
    if not target_case and CASE_MO_VECTORS:
        target_case = next(iter(CASE_MO_VECTORS.values()))
