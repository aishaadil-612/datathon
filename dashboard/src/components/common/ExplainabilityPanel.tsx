...
export const ExplainabilityPanel: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/75 backdrop-blur-sm flex justify-end transition-opacity animate-in fade-in duration-200 select-none">
      <div className="w-full max-w-xl bg-[#14151B] h-full shadow-2xl flex flex-col border-l border-[#22242D] overflow-hidden">
        {/* Drawer Header */}
        <div className="bg-[#0B0C0E] text-[#FFFFFF] px-6 py-4 flex items-center justify-between border-b border-[#22242D]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-lg font-bold tracking-tight text-[#FFFFFF]">{t.expTitle}</h2>
              <p className="text-xs text-[#9FA4B2] font-mono">CrimeLens Explainable AI Engine v3.2</p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#0B0C0E]">
          ...
          <div className="bg-[#14151B] p-5 rounded-[20px] border border-[#22242D] shadow-command space-y-2">
            ...
            <p className="text-[#FFFFFF] font-medium text-sm leading-relaxed">
              {data.conclusion}
            </p>
          </div>
          ...
          <div className={`p-5 rounded-[20px] border ${colors.border} ${colors.light} shadow-command space-y-3`}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-[#9FA4B2]">
                {t.expConfidence}
              </span>
            ...
            <div className="flex justify-between text-[11px] font-mono text-[#9FA4B2]">
              <span>Low (&lt;50%)</span>
          ...
          <div className="bg-[#14151B] p-5 rounded-[20px] border border-[#22242D] shadow-command space-y-3">
            <div className="flex items-center gap-2 text-xs font-bold text-[#9FA4B2] uppercase tracking-wider font-mono">
              <ChevronRight className="w-4 h-4 text-teal-400" />
              <span>{t.expReasoning}</span>
            </div>
            <div className="space-y-3 pt-1">
              {data.reasoningSteps.map((step, idx) => (
                <div key={idx} className="flex gap-3 text-xs text-[#FFFFFF] bg-[#0B0C0E] p-3.5 rounded-xl border border-[#22242D]">
                  ...
                  <p className="leading-relaxed text-[#FFFFFF]">{step}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {data.evidenceSources.map((source, idx) => (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-mono font-semibold bg-[#0B0C0E] text-teal-400 border border-[#22242D] hover:bg-[#1B1C24] transition-colors"
                >
                  <Database className="w-3.5 h-3.5 text-teal-400" />
                  <span>{source.label}</span>
                  <span className="text-[10px] text-[#9FA4B2] font-normal">({source.type})</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0B0C0E] text-[#FFFFFF] p-5 rounded-[20px] border border-[#22242D] space-y-3">
            <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-[#9FA4B2]">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs pt-1">
              <div className="bg-[#14151B] p-3 rounded-xl border border-[#22242D]">
                <span className="text-[#9FA4B2] block text-[10px] uppercase font-mono">Agent Name</span>
                <span className="font-bold text-[#FFFFFF]">{data.agentAttribution.name}</span>
              </div>
              <div className="bg-[#14151B] p-3 rounded-xl border border-[#22242D]">
                <span className="text-[#9FA4B2] block text-[10px] uppercase font-mono">Sub-System</span>
        ...
        {/* Footer */}
        <div className="px-6 py-4 bg-[#0B0C0E] border-t border-[#22242D] flex justify-between items-center text-xs text-[#9FA4B2] font-mono">
